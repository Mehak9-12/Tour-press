  ...
  return {
    ...
    protect: catchAsync(async (req, res, next) => {
      ...
      if (currentUser.changedPasswordAfter(decodedToken.iat)) {
        return next(
          new AppError(
            'User has recently changed the password, PPlease log in again',
            401,
          ),
        );
      }
    ...
    restrictTo: (...roles) => {
      return (req, res, next) => {
        console.log('user in restrictTo middleware', req.user);
        console.log(roles);
    ...
    forgotPassword: catchAsync(async (req, res, next) => {
      ...
      try {
        ...
        res.status(200).json({
          status: 'success',
          message:
            'Password reset link successfully sent on your email, Please check your inbox',
        });
      } catch (e) {
        console.log('🚀 ~ e:', e);
